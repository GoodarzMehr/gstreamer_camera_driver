...
namespace gscam {
  ...
  bool GSCam::configure()
  {
    ...
    if (!gsconfig_env && !gsconfig_rosparam_defined) {
      ...
      ROS_INFO_STREAM("Using GStreamer configuration from the environment: \"" << gsconfig_env << "\"");
    ...
    nh_private_.param("publish_timestamp", publish_timestamp_, false);
  ...
  bool GSCam::init_stream()
  {
    ...
    ROS_INFO("ROS time: %.3f", now.toSec());
    ROS_INFO("GStreamer time: %.3f", (GST_TIME_AS_USECONDS(ct) / 1e6));
